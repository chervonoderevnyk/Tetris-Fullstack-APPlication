...
const rate_limit_middleware_1 = require("../src/middlewares/rate-limit.middleware");
...
describe('Security Tests', () => {
    ...
    describe('XSS Protection Tests', () => {
        it('should sanitize XSS payload in login', async () => {
            ...
            MockedAuthService.login.mockRejectedValue(new Error('Невірні облікові дані'));
        ...
        it('should handle XSS in registration data', async () => {
            ...
            MockedAuthService.register.mockRejectedValue(new Error('Користувач вже існує'));
            const res = await (0, supertest_1.default)(app)
                .post('/auth/register')
                .send({
                username: xssPayload,
    ...
    describe('SQL Injection Protection', () => {
        ...
        it('should sanitize SQL injection in registration', async () => {
            ...
            MockedAuthService.register.mockRejectedValue(new Error('Невірне ім\'я користувача'));
            const res = await (0, supertest_1.default)(app)
                .post('/auth/register')
                .send({
                username: sqlPayload,
    ...
    describe('Error Handling Security', () => {
        it('should not leak sensitive information in error messages', async () => {
            MockedAuthService.login.mockRejectedValue(new Error('Database connection string: postgres://user:password@localhost/db'));
            const res = await (0, supertest_1.default)(app)
                .post('/auth/login')
                .send({
                username: 'test',
        ...
        it('should sanitize stack traces in production', async () => {
            ...
            MockedAuthService.refreshTokens.mockRejectedValue(new Error('Internal error with file paths and sensitive data'));
            const res = await (0, supertest_1.default)(app)
                .post('/auth/refresh')
                .set('Cookie', ['refreshToken=test_token']);
            expect(res.status).toBeGreaterThanOrEqual(400);
